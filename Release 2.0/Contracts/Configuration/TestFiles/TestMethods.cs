...
namespace Srushti.Updates
{
    internal class TestMethods
    {
        public void Test()
        {
            ...
            Configu<UpdateFiles>.Save(ufc, "c:\\log\\conf.xml");

            System.Xml.XmlDocument doc = new System.Xml.XmlDocument();
            doc.Load("c:\\log\\conf.xml");

            //UpdateFiles temp = UpdateFiles.Load(doc.InnerXml) as UpdateFiles;
            UpdateFiles temp = BaseXmlSerializer<UpdateFiles>.Load(doc.InnerXml) as UpdateFiles;
            ...
            BaseXmlSerializer<Product>.Save(p, "c:\\log\\prd.xml");

            doc = new System.Xml.XmlDocument();
            doc.Load("c:\\log\\prd.xml");
            //p = Product.Load(doc.InnerXml) as Product;
            p = BaseXmlSerializer<Product>.Load(doc.InnerXml) as Product;
